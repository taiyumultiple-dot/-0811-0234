 ...
 * 五門・心靈迷宮（生命教育推理遊戲）
 *
 * 這個遊戲是獨立製作的 HTML/CSS/JS，整包放在 public/lifequest/，
 * 透過 iframe 載入，不需要額外的建置流程——Vite 會把 public/ 底下的
 * 東西原封不動複製到 dist/。
 *
 * 注意：站內另有一支 MindLabyrinthGame.tsx，那是不同的遊戲（五章式、
 * WOOP 策略那個）。這一支是「五門・心靈迷宮」，玩法是把思考碎片
 * 排成鑰匙，兩者不要混淆。
 *
 * 路徑一定要用 import.meta.env.BASE_URL 開頭，
 * 部署到 GitHub Pages 的子路徑（/-0811-0234/）時才不會 404。
 */
import { useRef, useState } from 'react';
import { Home, Maximize2 } from 'lucide-react';

export default function FiveGatesGame({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div ref={wrapRef} className="fixed inset-0 z-50 bg-[#070B14] flex flex-col">
